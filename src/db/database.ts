...
class Database {
  ...
  private async transaction<T>(storeName: string, mode: IDBTransactionMode, fn: (store: IDBObjectStore) => IDBRequest<T> | void): Promise<T | void> {
  ...
  async getAllMedications(): Promise<Medication[]> {
    return (await this.transaction(STORE_NAME, 'readonly', store => store.getAll())) as Medication[];
  }

  async addMedication(medication: Medication): Promise<number> {
    const now = Date.now();
    const value = { ...medication, createdAt: medication.createdAt ?? now, updatedAt: now, history: medication.history ?? [] };
  ...
  async updateMedication(medication: Medication): Promise<void> {
    const value = { ...medication, updatedAt: Date.now(), history: medication.history ?? [] };
    await this.transaction(STORE_NAME, 'readwrite', store => store.put(value));
  }

  async deleteMedication(id: number): Promise<void> { await this.transaction(STORE_NAME, 'readwrite', store => store.delete(id)); }
  ...
  async replaceAllMedications(medications: Medication[]): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      store.clear();
      for (const medication of medications) store.put(medication);
  ...
  async exportBackup(): Promise<BackupPayload> {
    return { schemaVersion: DB_VERSION, exportedAt: Date.now(), medications: await this.getAllMedications() };
  }

  async importBackup(payload: BackupPayload): Promise<void> {
    if (!payload || !Array.isArray(payload.medications)) throw new Error('Invalid backup file');

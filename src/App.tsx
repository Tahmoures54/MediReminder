...
const scheduleNativeAlarms = async (meds: MedicationWithTimestamp[]) => {
  ...
  for (const med of meds) {
    if (med.running && med.remaining > 0) {
      // اعلان اصلی + چند یادآوری تکراری تا تأیید
      notifications.push({
        id: med.id!,
        title: '🔔 زمان مصرف دارو!',
      ...
      // یادآوری‌های تکراری پس از زمان اصلی (تا حدود ۱۵ دقیقه)
      for (let i = 1; i <= 20; i++) {
        const followId = Number(med.id) * 1000 + i;
        notifications.push({
          id: followId,
          title: '🔔 یادآوری مجدد دارو',
          body: `💊 ${med.name} — هنوز مصرف نکرده‌اید؟`,
          channelId: 'medication-alarms',
          schedule: {
            at: new Date(now + med.remaining * 1000 + i * FOLLOW_UP_NATIVE_SECONDS * 1000),
          },
          sound: 'medication_alarm.wav',
          iconColor: '#DC2626',
          extra: { medicationId: med.id },
  ...
  if (notifications.length > 0) {
    try {
      ...
      console.error('خطا در زمان‌بندی اعلان نیتیو:', err);
    }
  }
};

const cancelNativeFollowUps = async (medId: number) => {
  try {
    const toCancel = Array.from({ length: 20 }, (_, i) => ({
      id: Number(medId) * 1000 + (i + 1),
    }));
    toCancel.push({ id: Number(medId) });
...
export default function App() {
  ...
  const saveAllMedications = useCallback(async () => {
    ...
    try {
      ...
      console.error('خطا در ذخیره داروها:', error);
    } finally {
  ...
  const handleAddMedication = async (
    ...
    try {
      ...
      console.error('خطا در افزودن دارو:', error);
    }
  };
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          console.error('خطا در تغییر وضعیت دارو:', error);
        } finally {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          console.error('خطا در ریست تایمر:', error);
        } finally {
          setConfirmDialog(null);
        }
      },
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          if (Capacitor.isNativePlatform()) {
            await cancelNativeFollowUps(med.id!);
          } else {
            navigator.serviceWorker?.controller?.postMessage({
              type: 'CANCEL_ALARM',
              id: med.id,
  ...
  const handleRestartFromAlert = useCallback(async (alertMed: MedicationWithTimestamp) => {
    ...
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        await cancelNativeFollowUps(currentMed.id!);
      } else {
        navigator.serviceWorker?.controller?.postMessage({
          type: 'DISMISS_ALARM',
          id: currentMed.id,
      ...
      console.error('خطا در راه‌اندازی مجدد دارو:', error);
    }
  }, []);
  ...
  const handleCloseAlert = () => {
    setAlertQueue(prev => {
      ...
      // بستن پاپ‌آپ صدا را قطع نمی‌کند مگر اینکه «مصرف کردم» زده شود
      // فقط برای پیام‌های غیرآلارم دارو صدا قطع می‌شود
      const isMedAlert = current?.isMedicationAlert !== false && current?.medication?.id;
      ...
      if (newQueue.length === 0 && !isMedAlert) {
        stopAlarm();
      }
      // اگر آلارم دارو بود و فقط «بعداً» زد، صدا ادامه دارد تا مصرف یا اسنوز
      return newQueue;
    });
  };
  ...
  const handleSnooze = (minutes: number) => {
    ...
    if (!Capacitor.isNativePlatform()) {
      ...
      void cancelNativeFollowUps(Number(medId));

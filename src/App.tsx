...
import { normalize, toDue } from './utils/medication';
    ...
    const tick = async () => {
      ...
      const next = current.map((m) => {
        ...
        if (n.running && n.remaining !== m.remaining) {
          changed = true;
          return n; // remaining is derived for display — never persist ticks
        }
        return n;
      });
      if (changed) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-950 p-6 text-white">
        <div className="w-full max-w-md rounded-2xl border border-red-500/30 bg-gray-900 p-6 text-center shadow-xl">
          ...
          <p className="mt-3 break-words rounded-lg bg-gray-800 p-2 text-xs text-gray-500">
            {bootError}
          </p>
          <button
            type="button"
            ...
            className="mt-5 w-full rounded-xl bg-cyan-500 px-4 py-3 font-bold text-gray-950 hover:bg-cyan-400"
          >
            تلاش مجدد
          </button>
        </div>
      ...
      <div className="mx-auto max-w-lg px-4 py-6">
        <header className="mb-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <label className="rounded-xl border border-gray-600 bg-gray-800 px-4 py-3 text-sm font-semibold text-gray-200 hover:bg-gray-700 cursor-pointer">
              ورود پشتیبان
              <input
                type="file"
